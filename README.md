...
- **漢字表示問題の解決**：`matplotlib` のフォント設定を最適化することで、中国語文字の文字化けや欠落問題を解決し、認識結果を正確に可視化できるようにしました。  
...
- `matplotlib` の中国語フォント設定は、実行環境によって追加設定が必要な場合があります。  
...
- `ch_sim`（簡体字中国語）および `en`（英語）モデルを読み込みます。  
...
中国語文字を `matplotlib` 上で正しく表示するため、以下の設定を行っています：
